    ...
    <div className="absolute top-full left-0 mt-1 bg-white rounded-lg shadow-lg overflow-hidden z-30 min-w-[120px]">
      <ul className="py-1">
        {menuOptions.map((option) => (
          <li
            key={option}
            className="px-4 py-2 hover:bg-gray-100 cursor-pointer text-gray-800 text-sm font-medium"
...
const OptionTag = memo(({
  ...
  >
    ...
    <button
      ...
      className="h-4 w-4 flex items-center justify-center rounded-full hover:bg-white/20 text-white/70"
    >
      <svg
        width="10"
        height="10"
  ...
  return (
    <div className="flex-1 relative h-full flex items-center">
      <textarea
        ...
        rows={1}
        className="w-full min-h-8 max-h-24 bg-transparent text-sm font-normal text-left self-center text-white placeholder-gray-400 border-0 outline-none px-3 pr-10 py-1 z-20 relative resize-none overflow-y-auto"
        style={{
    ...
    <ChatInputContext.Provider value={contextValue}>
      ...
      >
        <div
          ...
          onClick={handleClick}
          className={`relative flex flex-col w-full min-h-full ${backgroundClass} backdrop-blur-xl shadow-lg rounded-3xl p-2 overflow-visible group transition-all`}
          style={{
            boxShadow:
